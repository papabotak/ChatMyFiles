...
class RAGWorkflow:
    ...
    def __init__(self, model=MODEL_LLAMA, db_name="vector_db"):
        self.model = model
    ...
    def create_vectorstore(self):
        ...
        self.vectorstore = FAISS.from_documents(
            ...
            distance_strategy="cosine"  # Faster than euclidean distance
        )
    ...
    def setup_chat(self):
        ...
        # 1.5 Use Ollama model for data privacy
        llm = Ollama(
            model=self.model,
            temperature=0.1, # Lower = faster, Higher = more accurate
            base_url="http://localhost:11434",
            timeout=60,
            # Performance optimization parameters
            num_ctx=4096,   # Context window(balance between speed and accuracy
            num_predict=512,  # Limit response length
            repeat_penalty=1.1, # Prevent repetition (faster generation)
            top_p=0.9,       # Sampling strategy. Higher = more diverse. Lower = more accurate
            top_k=40,       # Top-k sampling, limit token selection. Higher = more diverse. Lower = more accurate
            num_thread=os.cpu_count(),  # Use multiple CPU threads for faster generation
            #num_gpu=1,     # Use GPU if available
            mirostat=2,     # Better sampling efficiency
        )
